...
import { pickInputImages } from "../../services/files";
import { runJob } from "../../services/engine";
import type { JobResult } from "../../types/job";

// Same "no live thumbnails" tradeoff as JpgToPdfWorkspace: the asset
// protocol is scoped to the app's cache dir, not arbitrary picker paths.

function fileNameOf(path: string): string {
  return path.split(/[\\/]/).pop() ?? path;
}

export function ScanImportWorkspace({ onBack }: { onBack: () => void }) {
  const [files, setFiles] = useState<string[]>([]);
  const [enhance, setEnhance] = useState(true);
  const [outputDir, setOutputDir] = useState<string | null>(null);
  const [outputName, setOutputName] = useState("scanned.pdf");
  const [running, setRunning] = useState(false);
  const [result, setResult] = useState<JobResult | null>(null);

  async function addFiles() {
    const paths = await pickInputImages();
    const additions = paths.filter((p) => !files.includes(p));
  ...
  async function run() {
    ...
    try {
      const jobResult = await runJob({
        ...
        tool: "scan-import",
        inputs: files,
        options: { enhance, outputFilename: outputName },
        outputDir,
      });
      setResult(jobResult);
    } finally {
  ...
  return (
    <WorkspaceLayout title="Import Scan" onBack={onBack}>
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-medium text-slate-700 dark:text-slate-300">
            Scanned photos (in page order)
          </h2>
          <button
            type="button"
            onClick={addFiles}
        ...
        {files.length === 0 ? (
          <p className="py-10 text-center text-sm text-slate-400">
            Add one or more photos of a scanned document.
          </p>
        ) : (
      ...
      <section className="space-y-3">
        ...
        <label className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
          ...
          Clean up scan (grayscale + contrast boost)
        </label>
