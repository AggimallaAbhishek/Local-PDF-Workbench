...
// Thumbnails aren't rendered here: the asset protocol is deliberately scoped
// to the app's own cache dir (see tauri.conf.json), not arbitrary picker
// paths, so an image the user just chose can't be loaded into an <img> tag
// without widening that scope. Filenames + order are enough to merge correctly.

function fileNameOf(path: string): string {
  return path.split(/[\\/]/).pop() ?? path;
}

export function JpgToPdfWorkspace({ onBack }: { onBack: () => void }) {
  ...
  return (
    <WorkspaceLayout title="JPG to PDF" onBack={onBack}>
      <section className="space-y-3">
        ...
        {files.length === 0 ? (
          <p className="py-10 text-center text-sm text-slate-400">
            Add one or more JPG/PNG images to combine into a PDF.
          </p>
        ) : (
